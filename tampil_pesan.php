					...
					<hr>
						<table class="table table-bordered">
							...
							<?php
								...
								//Asign a query
								$query = " SELECT id_tamu, nama_tamu, subjek, email, tanggal, balas FROM tamu ORDER BY id_tamu DESC LIMIT $posisi,$batas ";
								// Execute the query
								...
								while ($row = $result->fetch_object()){
									...
									/*$balas = test_input($row->balas);
									if(EMPTY($balas) || $balas ==" " || $balas =="" || $balas == null){
										echo '<td>Pesan Belum Dibalas</td>';
									}else{
										echo '<td>Pesan Sudah Dibalas</td>';
									}*/
									echo '<td>
											<a href="detail_pesan.php?id_tamu='.$row->id_tamu.'"><img src="../img/loop.png"></a></td>';?>
									<td>
											<a href="<?php echo "hapus_pesan.php?id_tamu=$row->id_tamu";?>" onclick=" return confirm('Anda Yakin ingin Menghapus Data Tersebut?')"><img src="../img/delete.png"></a>
								     </td>
									 <?php
									echo '</tr>';
									$i++;
									...
									$query2     = mysqli_query($db, "SELECT id_tamu, nama_tamu, subjek, email, tanggal FROM tamu ORDER BY id_tamu");
									...
									for($n=1;$n<=$jmlhalaman;$n++)
										if ($n != $halaman){
											echo " <a href=\"tampil_pesan.php?halaman=$n\">$n</a> ";
										}
										else{
